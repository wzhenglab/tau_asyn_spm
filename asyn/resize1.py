...
spm_gap=2
cx=-(box_length/2)+10 # far away from protein
n_spm_y=4
for i in range(n_spm):
...
system.replicate(nx=2,ny=3,nz=1)
#Bonds
...
resize=hoomd.update.box_resize(Lx=hoomd.variant.linear_interp([(0, system.box.Lx), (1e6, 18)]), Ly=hoomd.variant.linear_interp([(0, system.box.Ly), (1e6, 18)]),Lz=hoomd.variant.linear_interp([(0, system.box.Lz), (1e6, 18)]) )

hoomd.dump.dcd('resize1.dcd', period=20000, group=all, overwrite=True)
hoomd.dump.gsd('resize1.gsd', period=20000, group=all, truncate=True, overwrite=True)

## Run simulation                                                               
hoomd.run(tsteps=1e6+1e5)   
