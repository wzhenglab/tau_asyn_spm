...
## Outputs
hoomd.analyze.log('init.log', quantities=['bond_harmonic_energy','angle_table_energy','pair_ashbaugh_energy','pair_yukawa_energy','potential_energy','kinetic_energy','temperature'], period=10000, overwrite=True, header_prefix='#')
hoomd.dump.gsd('restart_init-1.gsd', period=1000000, group=all, truncate=True)    
hoomd.dump.gsd('restart_init-2.gsd', period=1000000, group=all, truncate=True, phase=500000)
...
hoomd.dump.dcd('init.dcd', period=20000, group=all, overwrite=True)
hoomd.dump.gsd('init.gsd', period=20000, group=all, truncate=True, overwrite=True)
