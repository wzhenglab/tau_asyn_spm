...
# #### 1.3 Build input gsd file for resize1ial conformation and coordinate
...
bond_length=0.38  # nm
box_length=bond_length*chain_length+80
...
# Build resize1ial position as a linear chain
pos=[]
cy=-box_length/2+0.1
cz=0
...
system = hoomd.init.read_gsd('init.gsd',time_step=0)
system.replicate(nx=3,ny=2,nz=1)
...
## Set up integrator
hoomd.md.integrate.mode_standard(dt=0.001) # Time units in ps
#Temperature=298  # K
...
## Outputs
#hoomd.analyze.log('resize1.log', quantities=['bond_harmonic_energy','angle_table_energy','pair_ashbaugh_energy','pair_yukawa_energy','potential_energy','kinetic_energy','temperature'], period=10000, overwrite=True, header_prefix='#')
hoomd.analyze.log('resize1.log', quantities=['bond_harmonic_energy','pair_ashbaugh_energy','pair_yukawa_energy','potential_energy','kinetic_energy','temperature','lx','ly','lz'], period=10000, overwrite=True, header_prefix='#')
hoomd.dump.gsd('restart_resize1-1.gsd', period=1000000, group=all, truncate=True)    
hoomd.dump.gsd('restart_resize1-2.gsd', period=1000000, group=all, truncate=True, phase=500000)
#old.dump.xml(all,fileroot+'.xml', period=1000000)                              
resize=hoomd.update.box_resize(Lx=hoomd.variant.linear_interp([(0, system.box.Lx), (2e7, 28)]), Ly=hoomd.variant.linear_interp([(0, system.box.Ly), (2e7, 28)]),Lz=hoomd.variant.linear_interp([(0, system.box.Lz), (2e7, 28)]) )

hoomd.dump.dcd('resize1.dcd', period=10000, group=all, overwrite=True)
hoomd.dump.gsd('resize1.gsd', period=10000, group=all, truncate=True, overwrite=True)
