...
# #### 1.3 Build input gsd file for resize2ial conformation and coordinate
...
# Build resize2ial position as a linear chain
pos=[]
cy=-box_length/2+0.1
cz=0
...
# build spermine resize2ial coordinates
...
system = hoomd.init.read_gsd('resize1.gsd',time_step=0)
...
## Set up integrator
hoomd.md.integrate.mode_standard(dt=0.002) # Time units in ps
#Temperature=298  # K
...
## Outputs
#hoomd.analyze.log('resize2.log', quantities=['bond_harmonic_energy','angle_table_energy','pair_ashbaugh_energy','pair_yukawa_energy','potential_energy','kinetic_energy','temperature'], period=10000, overwrite=True, header_prefix='#')
hoomd.analyze.log('resize2.log', quantities=['bond_harmonic_energy','pair_ashbaugh_energy','pair_yukawa_energy','potential_energy','kinetic_energy','temperature','lx','ly','lz'], period=10000, overwrite=True, header_prefix='#')
hoomd.dump.gsd('restart_resize2-1.gsd', period=1000000, group=all, truncate=True)    
hoomd.dump.gsd('restart_resize2-2.gsd', period=1000000, group=all, truncate=True, phase=500000)
resize=hoomd.update.box_resize(Lx=hoomd.variant.linear_interp([(0, system.box.Lx), (2e7, 26)]), Ly=hoomd.variant.linear_interp([(0, system.box.Ly), (2e7, 26)]),Lz=hoomd.variant.linear_interp([(0, system.box.Lz), (2e7, 26)]) )

hoomd.dump.dcd('resize2.dcd', period=10000, group=all, overwrite=True)
hoomd.dump.gsd('resize2.gsd', period=10000, group=all, truncate=True, overwrite=True)
